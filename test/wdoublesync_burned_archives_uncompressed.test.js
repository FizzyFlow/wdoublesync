/**
 * Tests for WDoubleSync with burned archives - UNCOMPRESSED variant.
 *
 * Same tests as wdoublesync_burned_archives.test.js but with gzip compression disabled.
 * This helps isolate whether compression is causing any issues with snapshot restoration.
 *
 * Covers:
...
// ─── burned archives tests (uncompressed) ───────────────────────────────────────

describe('burned archives (uncompressed)', () => {
    it('initializes correctly after multiple pushes', async () => {
        const ev = await createEV();
        const w = new WDoubleSync({ endlessVector: ev, sync: makeSync(), compress: false });
        ...
        const w2 = new WDoubleSync({ endlessVector: ev2, sync: makeSync(), compress: false });
        await w2.initialize();
    ...
    it('can force fresh snapshot for rebate workflow', async () => {
        ...
        const w3 = new WDoubleSync({ endlessVector: ev3, sync: makeSync(), compress: false });
